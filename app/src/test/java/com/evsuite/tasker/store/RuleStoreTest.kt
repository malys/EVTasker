...
class RuleStoreTest {
    ...
    fun `a rule naming a catalog entry that no longer exists is dropped, not crashed on`() {
        ...
        val json = context().getSharedPreferences("ev_tasker_rules", android.content.Context.MODE_PRIVATE)
        ...
        context().getSharedPreferences("ev_tasker_rules", android.content.Context.MODE_PRIVATE)
    ...
    fun `a removed catalog entry inside another case drops the rule too`() {
        ...
        store.save(sampleRule("still valid"))
        val prefs = context()
            .getSharedPreferences("ev_tasker_rules", android.content.Context.MODE_PRIVATE)
