...
import com.evsuite.tasker.model.DELAY_BUDGET_MS
...
class RuleEngine(
    ...
    private fun fire(rule: Rule, actions: List<Action>, branch: Int?): RuleRun {
        ...
        for (action in actions) {
            val result =
                if (action.type == ActionType.DELAY) pause(action) else executeWithBackoff(action)
            results += result
            // A declined confirmation ends the branch where it stands — that is what the
            // action is for. The actions after it are not reported: nothing was attempted,
            // and a history row for an action that never ran would have no verdict to give.
            if (result.verdict == BridgeContract.VERDICT_DECLINED) break
        }
