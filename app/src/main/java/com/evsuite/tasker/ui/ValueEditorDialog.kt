...
import com.evsuite.tasker.databinding.DialogValueEditorBinding
import com.evsuite.tasker.databinding.ScreenValueEditorBinding
 ...
 *
 * A single dialog drives every type: it reads the [com.evsuite.hardware.catalog.ValueSpec] and
...
object ValueEditorDialog {
        ...
        val state = bindValue(
            ...
            choices = when {
                condition.type == ConditionType.FIRMWARE_GEN ->
                    com.evsuite.hardware.catalog.VehicleEnums.FIRMWARE_GENS.map { Choice(it, it) }
    ...
    private fun showEditor(
        ...
        val dialog = android.app.Dialog(context, R.style.Theme_EV_Picker)
    ...
    private fun bindValue(
        ...
        if (gated) {
            ...
            binding.gatedExplain.setText(R.string.editor_gated_explain)
        }
        ...
        when (spec.kind) {
            ...
            ValueKind.LOCATION -> {
                text = bindPoint(
                    context = context,
                    binding = binding,
                    hintRes = R.string.value_location_hint,
                    initialText = initialText,
                    currentPoint = currentPoint,
                    // The place the rule is about is almost always the place the car is at
                    // when the rule is written, so the field opens on it.
                    prefillWithCurrent = true
                ) { text = it }
            ...
            /**
             * The same point controls as [ValueKind.LOCATION], minus the radius — a
             * destination is a place, not an area — and without the prefill: the car's
             * current position is where the driver already is, which is the one destination
             * a rule never means. It stays one button away.
             */
            ValueKind.DESTINATION -> {
                text = bindPoint(
                    context = context,
                    binding = binding,
                    hintRes = R.string.value_destination_hint,
                    initialText = initialText,
                    currentPoint = currentPoint,
    ...
    /**
     * The text field plus everything that fills it without typing: the car's position, the
     * saved places, and the map.
     *
     * Shared by the location condition and the navigation action because the input is the
     * same input — "a point" — and the difference between them is only whether a radius
     * follows and whether "here" is a sensible default.
     *
     * @return the value the field opens on, so the caller can seed its own state with it.
     */
    private fun bindPoint(
        ...
        initialText: String,
        currentPoint: String?,
        prefillWithCurrent: Boolean,
        onPoint: (String) -> Unit
    ): String {
        binding.textBlock.visibility = View.VISIBLE
        binding.textInput.hint = context.getString(hintRes)
        val start = if (prefillWithCurrent) initialText.ifBlank { currentPoint.orEmpty() } else initialText
        binding.textInput.setText(start)
        binding.textInput.addTextChangedListener(onPoint)

        bindPlaces(context, binding)
        ...
        binding.locationCurrent.setOnClickListener {
            ...
            com.evsuite.tasker.util.CarLocation.requestCurrent(context) { fix ->
                binding.locationCurrent.isEnabled = true
                if (fix != null) {
                    binding.textInput.setText(formatPoint(fix.latitude, fix.longitude))
                }
            }
        }
        binding.locationMap.setOnClickListener {
            val centre = binding.textInput.text?.toString().orEmpty().ifBlank { currentPoint.orEmpty() }
            ...
            if (com.evsuite.tasker.util.MapApps.open(context, centre) == null) {
                Toast.makeText(context, R.string.value_location_map_none, Toast.LENGTH_SHORT).show()
            }
        }
        binding.locationSave.setOnClickListener {
            promptSavePlace(context, binding.textInput.text?.toString().orEmpty()) {
                bindPlaces(context, binding)
            }
        }
        return start
    ...
    /**
     * Fills the saved-places dropdown, or hides it when there is nothing saved yet.
     *
     * Called again after a save so the place just named is immediately pickable, which is
     * also why the whole spinner is rebuilt rather than the adapter mutated: it has to
     * re-appear the first time, and one path is easier to be sure of than two.
     */
    private fun bindPlaces(context: Context, binding: DialogValueEditorBinding) {
        val places = com.evsuite.tasker.store.PlaceStore.all(context)
        if (places.isEmpty()) {
            binding.placeSpinner.visibility = View.GONE
            return
        }
        // A "pick a place" row first: without it the spinner would claim the driver had
        // chosen its first entry, and selecting that entry on purpose would fire nothing.
        val labels = listOf(context.getString(R.string.value_place_pick)) + places.map { it.name }
        binding.placeSpinner.visibility = View.VISIBLE
        binding.placeSpinner.adapter = simpleAdapter(context, labels)
        binding.placeSpinner.setSelection(0)
        binding.placeSpinner.onItemSelected { index ->
            if (index > 0) binding.textInput.setText(places[index - 1].point)
        }
    }

    /** Asks for a name and stores [point] under it. [onSaved] runs only on a real save. */
    private fun promptSavePlace(context: Context, point: String, onSaved: () -> Unit) {
        if (point.isBlank()) {
            Toast.makeText(context, R.string.value_place_empty, Toast.LENGTH_SHORT).show()
            return
        }
        val input = com.google.android.material.textfield.TextInputEditText(context).apply {
            hint = context.getString(R.string.value_place_name)
        ...
        com.google.android.material.dialog.MaterialAlertDialogBuilder(context)
            .setTitle(R.string.value_place_save)
            .setView(frame)
            .setNegativeButton(android.R.string.cancel, null)
            .setPositiveButton(android.R.string.ok) { _, _ ->
                val name = input.text?.toString().orEmpty()
                if (com.evsuite.tasker.store.PlaceStore.save(context, name, point)) {
                    onSaved()
                } else {
                    Toast.makeText(context, R.string.value_place_name, Toast.LENGTH_SHORT).show()
                }
            }
            .show()
    }

    private fun formatPoint(latitude: Double, longitude: Double): String =
