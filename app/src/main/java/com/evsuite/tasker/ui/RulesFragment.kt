...
import com.evsuite.tasker.databinding.FragmentRulesBinding
import com.evsuite.tasker.model.EngineRun
import com.evsuite.tasker.model.Rule
import com.evsuite.tasker.service.TaskerRunService
...
import com.evsuite.tasker.vehicle.CycleReporter
...
class RulesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.automationSwitch.setOnCheckedChangeListener { _, checked ->
            if (checked && !AppState.hasCurrentWriteConsent(requireContext())) {
                binding.automationSwitch.isChecked = false
                showWriteConsent()
            } else {
    ...
    private fun showWriteConsent() {
        MaterialAlertDialogBuilder(requireContext())
            .setTitle(R.string.write_consent_title)
            .setMessage(R.string.write_consent_message)
            .setNegativeButton(R.string.write_consent_cancel, null)
            .setPositiveButton(R.string.write_consent_accept) { _, _ ->
                AppState.acceptCurrentWriteConsent(requireContext())
                AppState.setAutomationEnabled(requireContext(), true)
                binding.automationSwitch.isChecked = true
            }
            .show()
    }
     ...
     */
    private fun renderOtherCases(rule: com.evsuite.tasker.model.Rule, labels: Labels) {
        binding.detailCases.removeAllViews()
        val inflater = layoutInflater

        rule.elseIfBranches.forEachIndexed { index, branch ->
            val case = com.evsuite.tasker.databinding.ItemDetailCaseBinding
