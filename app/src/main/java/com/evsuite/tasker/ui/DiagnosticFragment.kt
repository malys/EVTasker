...
import com.evsuite.tasker.databinding.ItemDiagnosticBinding
import com.evsuite.tasker.databinding.ItemDiagnosticHeaderBinding
import com.evsuite.tasker.debug.DebugReport
import com.evsuite.tasker.debug.DiagnosticProbe
...
/**
 * What EVTasker can actually do on THIS vehicle, right now.
 *
...
class DiagnosticFragment : Fragment() {
    ...
    private fun load() {
        ...
        // The probe blocks: reflection, system properties, and a bind to EVProfile with a
    ...
    private fun envLabel(id: DiagnosticProbe.Env): String = getString(
        when (id) {
            ...
            DiagnosticProbe.Env.EVPROFILE -> R.string.diag_env_evprofile
    ...
    private fun envValueRes(check: DiagnosticProbe.EnvCheck): Int = (
        when (check.id) {
            ...
            // EVProfile is reported, never judged: absent is the normal, supported case.
            DiagnosticProbe.Env.EVPROFILE ->
                if (check.detail == DiagnosticProbe.EVPROFILE_INSTALLED) R.string.diag_installed
    ...
    private fun color(ok: Boolean) =
        ContextCompat.getColor(requireContext(), if (ok) R.color.ev_ok else R.color.ev_error)
