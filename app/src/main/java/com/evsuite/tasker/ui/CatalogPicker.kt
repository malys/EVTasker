...
import com.evsuite.tasker.databinding.DialogCatalogPickerBinding
import com.evsuite.tasker.databinding.ItemCatalogEntryBinding
import com.evsuite.tasker.databinding.ItemCatalogGroupBinding
...
import com.evsuite.tasker.util.MapApps
 ...
 * back to the live matrix filter on [firmware]. A null firmware (no bridge, or EVProfile
...
object CatalogPicker {
    ...
    fun pickAction(context: Context, firmware: FirmwareGen?, onPick: (ActionType) -> Unit) {
        ...
        val debug = EmulatorDetector.isEmulator()
        // The profile actions are the ones that cannot work without EVProfile. Offering them
        // with an empty profile list only produces a rule that fails at ignition.
        val hasProfiles = ProfileBridge.isEVProfileInstalled(context)
    ...
    private fun show(context: Context, title: String, groups: List<Group>) {
        ...
        val dialog = Dialog(context, R.style.Theme_EV_Picker)
