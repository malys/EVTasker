...
object SupportStore {
    ...
    private const val PREFS = "ev_tasker_support"
    ...
    /**
     * The record, or null when it predates this build.
     *
     * [known] already drops names an update removed. The opposite case needs this: a set
     * saved by an earlier version cannot name an entry that version did not have, and the
     * rule editor reads a missing name as "not supported here". A new catalogue entry would
     * therefore be invisible on every car that had ever run a check — hidden by a record
     * that was never asked about it. Treating a stale record as no record puts the editor
     * back on the live firmware matrix until the next check overwrites it.
     */
    private fun stored(context: Context, key: String): Set<String>? {
        if (needsCheck(context)) return null
        return prefs(context).takeIf { it.contains(key) }?.getStringSet(key, emptySet())?.toSet()
    }
