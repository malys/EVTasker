...
object SupportStore {
    ...
    fun supportedConditions(context: Context): Set<String>? =
        prefs(context).takeIf { it.contains(KEY_CONDITIONS) }
            ?.getStringSet(KEY_CONDITIONS, emptySet())?.toSet()

    fun supportedActions(context: Context): Set<String>? =
        prefs(context).takeIf { it.contains(KEY_ACTIONS) }
            ?.getStringSet(KEY_ACTIONS, emptySet())?.toSet()
