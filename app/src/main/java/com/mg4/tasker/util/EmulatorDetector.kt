 ...
 * real head unit.
 *
 * Used by [com.mg4.tasker.vehicle.DirectExecutor] to bypass the firmware-confirmation gate:
 * no emulator image runs SAIC firmware, so [com.mg4.hardware.FirmwareInfo] never resolves a
 * generation there and every gated action would otherwise be refused before it even reaches
 * MG4Hardware. Bypassing the gate lets the underlying getters/setters run for real — on the
